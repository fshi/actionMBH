 ...
 *
 * Lear/INRIA/Dalal's HOG binary file iutput implementations 
...
#define LSGENERAL(TYPE)  \
	BiIStream& operator>>(TYPE& buffer){ \
		...
		BiIStream& read(TYPE* buffer, signed long size) {\
			from.read((char*)buffer, size*sizeof(TYPE)); \
			return *this;\
		}
...
#undef LSGENERAL
	...
	BiIStream& operator>>(std::string& buffer){ 
        ...
        bool inline eof() const { return from.eof(); }
        bool inline fail() const { return from.fail(); }
        bool inline bad() const { return from.bad(); }

        bool inline operator!() const { return !from; }
        ...
        BiIStream& seekg(pos_type p,std::ios_base::seekdir dir) { 
            from.seekg(p,dir);
            return *this;
        }
