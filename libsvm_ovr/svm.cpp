...
private:
	...
	{
		return exp(-gamma*chisquared(x[i],x[j]));
	}
...
double Kernel::intersection(const svm_node *px, const svm_node *py)
{
	...
	/*
	//no need because if px->index != -1 || py->index != -1, either px->value or py->value is equal to zero
	while(px->index != -1)
	{
		++px;
	}
...
double Kernel::chisquared(const svm_node *px, const svm_node *py)
{
	...
	while(px->index != -1 && py->index != -1)
	{
		if(px->index == py->index)
		{
			diff = px->value - py->value;
			add = px->value + py->value;
			if(add > 0)
				sum += (diff * diff / add);
			++px;
			++py;
			  ...
			  const svm_parameter& param)
{
	switch(param.kernel_type)
	{
		...
		case CHISQUARED: 
			return exp(-param.gamma*chisquared(x,y));
...
static void svm_group_classes(const svm_problem *prob, int *nr_class_ret, int **label_ret, int **start_ret, int **count_ret, int *perm)
{
	...
	*start_ret = start;
	*count_ret = count;
	//std::cout<<l<<" "<<label[0]<<" "<<label[10]<<" "<<label[70]<<" "<<label[80]<<std::endl;
	free(data_label);
}
...
svm_model *svm_train(const svm_problem *prob, const svm_parameter *param)
{
	...
	else
	{
		...
		{	
			...
			//std::cout<<param->weight[i]<<std::endl;
		}
...
int svm_save_model(const char *model_file_name, const svm_model *model)
{
	...
	if(param.kernel_type == POLY || param.kernel_type == RBF || param.kernel_type == SIGMOID || param.kernel_type == CHISQUARED)
		fprintf(fp,"gamma %g\n", param.gamma);
